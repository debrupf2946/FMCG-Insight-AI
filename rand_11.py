...
model_shelf_life = load_model('yolo_weights/shelf_life_generalized_model.h5')
...
# Get the video properties (for saving the result)
...
output_video = cv2.VideoWriter('output_video_with_detection.mp4', fourcc, fps, (frame_width, frame_height))
...
while True:
    ...
    # Save the frame with detection and shelf-life predictions to the output video
    output_video.write(frame)

    # Optionally, you can still display the frame (remove cv2.imshow if not needed)
...
print("Video saved as output_video_with_detection.mp4")
